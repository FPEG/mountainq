...
uint8 MPU6050_Init(void)
{
	...
	if(res==MPU6050_ADRESS)// 器件ID正确
	{
	       ...
	       //		MPU6050_Offset();
	       //Balance_Init();
	       MPU6050_WR(MPU_PWR_MGMT2_REG,0X00);	//?ó?ù?èó?íó?Yò???1¤×÷
	       MPU6050_WR(MPU_SAMPLE_RATE_REG,0X00);	      // 设置MPU6050的采样率,1KHz				//éè??2é?ù?ê?a50Hz
	       return 0;
	}
	else
	{
...
//===============================四元素============================================
#define Kp_1 1.6f //10.0f             	// proportional gain governs rate of convergence to accelerometer/magnetometer
#define Ki_1 0.001f//1.2f // //0.008f  	// integral gain governs rate of convergence of gyroscope biases
...
void IMUupdate(float gx, float gy, float gz, float ax, float ay, float az)
{
	...
	exInt = exInt + ex * Ki_1;	// 对误差进行积分
	eyInt = eyInt + ey * Ki_1;
	ezInt = ezInt + ez * Ki_1;

	// adjusted gyroscope measurements
	gx = gx + Kp_1*ex + exInt;	// 将误差PI后补偿到陀螺仪，即补偿零点漂移
	gy = gy + Kp_1*ey + eyInt;
	gz = gz + Kp_1*ez + ezInt;	// 这里的gz由于没有观测者进行矫正会产生漂移，表现出来的就是积分自增或自减
