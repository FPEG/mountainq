...
/*
************************************************************************************************
* 函数：void TFT_SetXY(unsigned int Xpos, unsigned int Ypos)
* 参数：xy坐标
* 返回：0
* 功能：设置lcd显示起始点
************************************************************************************************
*/
void TFT_SetXY(unsigned int Xpos, unsigned int Ypos)
{
	Lcd_WriteIndex(0x2a);
...
/*
************************************************************************************************
* 函数：void TFT_DrawPoint(unsigned int x,unsigned int y,Color_TypeDef Data)
* 参数：xy坐标和颜色数据
* 返回：0
* 功能：画一个点
simple useage:  TFT_DrawPoint(1,1,RED);
************************************************************************************************
*/
void TFT_DrawPoint(unsigned int x,unsigned int y,uint16 color)//0.019ms
{
	TFT_SetXY(x,y);
	LCD_WriteData_16Bit(color);
}
...
void TFT_showstr(uint16 x, uint16 y, uint8 dat[], uint16 PenColor, uint16 BGColor)
{
	...
	while (dat[j] != '\0')
	{
		TFT_showchar(x + 8 * j, y * 16, dat[j],PenColor,BGColor);
		j++;
	}
}
...
void TFT_showint8(uint16 x, uint16 y, int8 dat, uint16 PenColor, uint16 BGColor)
{
	...
	if (dat<0)
	{
		TFT_showchar(x, y * 16, '-', PenColor, BGColor);
		dat = -dat;
	}
	else	TFT_showchar(x, y * 16, ' ', PenColor, BGColor);
	...
	while (i<3)
	{
		TFT_showchar(x + (8 * i), y * 16, '0' + a[i], PenColor, BGColor);
		i++;
	}
...
void ShowImage(uint16 x, uint16 y, uint8 Image[80][160],uint16 col, uint16 row, uint16 PenColor, uint16 BGColor)
{
       ...
       for (i = 0; i<col; i++)
       {
	      Lcd_SetRegion(x, y + i, x + 79, y + i);
	      //temp = tft_ascii[dat - 32][i];//减32因为是取模是从空格开始取得 空格在ascii中序号是32
	      for (j = 0; j<row; j++)
	      {
		     if (Image[j][col-1-i] & 0x01)	LCD_WriteData_16Bit(PenColor);
		     else			LCD_WriteData_16Bit(BGColor);
		     //temp >>= 1;
	      }
       }
...
//  @return     void
//  @since      v1.0
//  Sample usage:   displayimage032(image[0]);
//-------------------------------------------------------------------------------------------------------------------
...
//二值化图像
void displayimage_binaryzation(uint16 x, uint16 y, uint8 *p,uint16 row,uint16 col,  uint16 PenColor, uint16 BGColor)  //右上角开始
{
       uint8 i, j;
       //uint8 temp;
       for (i = 0; i<row; i++)
       {
	      Lcd_SetRegion(y+ i,x, y + i, x + col-1);
	      //temp = tft_ascii[dat - 32][i];//减32因为是取模是从空格开始取得 空格在ascii中序号是32
	      for (j = 0; j<col; j++)
	      {
		     if (*(p + i*col + (col-1)-j) & 0x01)	LCD_WriteData_16Bit(PenColor);
		     else					LCD_WriteData_16Bit(BGColor);
		     //temp >>= 1;
	      }
       }
...
void displayimage032(uint8 *p)
{
       ...
       for(i=0;i<80;i++)
       {
	      ...
	      for(j=0;j<160;j++)
	      {
		     temp = *(p + i*ROW/80*COL + (COL-1)-j*(COL-1)/(160-1));//读取像素点
