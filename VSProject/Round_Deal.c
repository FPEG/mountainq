...
////局部变量，紧用于本文件
unsigned char  Round_row=0;
unsigned char  Round_col=0;
int  Round_num=0;
unsigned char  LeftLoopTemp_R=0,LeftLoopTemp_C=0,Inflection_Temp_x,Inflection_Temp_y;
float  LeftLoopSlope;
...
* 函数名称： Round_Detection(Str_Boundary *str,Str_Round *round)
* 功能说明： 圆环检测
* 参数说明：
* 函数返回：voiold
* 修改时间：
* 备 注：     检测一直开着
*              圆环检测  1、判断一边有下拐点，一边没有下拐点，且拐点位置离结束行不不少于5行 2、拐点侧丢线 3非拐点侧直线
*************************************************************/
void Round_Detection(Str_Boundary *str,Str_Round *round) //圆环
{
       //左环
       //检测到左下拐点，没检测到右下拐点，结束行比拐点远很多
       if(Inflection_point.L_down_point.x!=0&&Inflection_point.R_down_point.x==0&&str->EndLine<Inflection_point.L_down_point.x-5)
       {
	      //左下拐点行的上面三行左边丢线
	      if(str->LeftEdge[Inflection_point.L_down_point.x-1]==0&&str->LeftEdge[Inflection_point.L_down_point.x-2]==0&&str->LeftEdge[Inflection_point.L_down_point.x-3]==0)
	      {
		     round->Entrance_count=0;//圆环入口计数清零
		     if(Inflection_point.L_down_point.x+5>ROW_END-1)
			    Round_row=ROW_END-1;
		     else
			    Round_row=Inflection_point.L_down_point.x+5;
		     for(;Round_row>Inflection_point.L_down_point.x-10&&Round_row>=ROW_START;Round_row--)//从左拐点下面5行到上面10行进行搜索
		     {
	    ...
	    case Left_Round:  //左环
	      {
		     switch(RoundProcess)
		     {
			  ...
			  case Find_Gap://发现圆环中心
			    {
				   /*下一阶段的判定*/
				   //底部三行左边界都不丢线
				   if(str->LeftEdge[ROW_END-3]&&str->LeftEdge[ROW_END-1]&&str->LeftEdge[ROW_END-2])
				   {
			  ...
			  case Into_Round://进环    关键 分两步
			    {
				   /*进环的补线处理*/
				   if(str->LeftEdge[ROW_END-1]&&str->LeftEdge[ROW_END-2])//&&str->LeftEdge[ROW-4])
				   {      //获取补线点信息
					  ...
					  {
						 if(image[i][Inflection_Temp_y]==White&&image[i-1][Inflection_Temp_y]==Black)
						 {
							LeftLoopTemp_R=i-1;
							str->EndLine=i-1;
							break;
					  ...
					  {
						 if(image[LeftLoopTemp_R-1][i+1]==White&&image[LeftLoopTemp_R-1][i]==Black)
						 {
							LeftLoopTemp_C=i;
							break;
						 }
				   ...
				   for(uint8 i=str->EndLine;i<ROW_END;i++)
				   {
					  ...
					  {
						 if(image[i][j]==White&&image[i][j+1]==Black)
						 {
			  ...
			  case Into_Round2://进环    关键 4
			    {
				   ...
				   for(uint8 i=str->EndLine;i<=ROW_END;i++)
				   {
					  for(uint8 j=1;j<=75;j++)
					  {
						 if(image[i][j]==White&&image[i][j+1]==Black&&image[i][j+2]==Black&&image[i][j+3]==Black)
						 {
							if(str->RightEdge[i]>j+1)
							       str->RightEdge[i]=j+1;
							break;
				   ...
				   {
					  if(image[i][10]==White&&image[i-1][10]==Black&&image[i-2][10]==Black)
					  {
			  ...
			  case In_Round://在环内    5
			    {
				   //记录上一次拐点存在的信息
				   if(Inflection_point.R_down_point.x&&image[Inflection_point.R_down_point.x][Inflection_point.R_down_point.y-2]==White&&image[Inflection_point.R_down_point.x][Inflection_point.R_down_point.y-3]==White&&image[Inflection_point.R_down_point.x-1][Inflection_point.R_down_point.y]==White&&image[Inflection_point.R_down_point.x-2][Inflection_point.R_down_point.y]==White)
				   {
					  ...
					  if(Inflection_point.R_down_point.x>5)
					  {
						 ...
						 for(Round_row=Inflection_point.R_down_point.x-1; Round_row>=Inflection_point.R_down_point.x-6&&Round_row>ROW_START ; Round_row-- )     //在搜索到的黑白跳变点后，向上搜索白点，判断是否为干扰点
						 {
						 ...
						 if( hang_num>1|| Inflection_point.R_down_point.x>25||Inflection_point.R_down_point.y>50)
						 {
...
//				   memset(str->LeftEdge, 0,sizeof(str->LeftEdge) );
			     ...
			     case Leave_Round://离开圆环
			    {
				   ...
				   if(Round_num<3&&((str->R_StartLine-str->EndLine)>32||str->RightEdge[str->EndLine]>15))  //30需要调  huo 判断斜率
				   {
